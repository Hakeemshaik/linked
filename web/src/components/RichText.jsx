...
/** Message text with :love: style codes drawn as the app's emoji. */
export default function RichText({ text = '' }) {
  const out = [];
  let last = 0;
  for (const m of text.matchAll(TOKEN)) {
    if (!EMOJI_IDS.has(m[1])) continue;
    out.push(text.slice(last, m.index));
    out.push(<img key={m.index} className="emo" src={emojiUrl(m[1])} alt={m[0]} title={m[1]} draggable="false" />);
