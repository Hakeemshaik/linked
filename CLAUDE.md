...
- `server/` is Node 22 and Express on Postgres, plus web-push (VAPID). The server is stateless: anything shared lives in the DB, so it runs as a Vercel function.
  ...
  - `features/calendar.js`: `busyByUser` loads everyone's busy time for a range in two queries; `gaps` finds free windows (never in the past). `/calendar/free` (with `times` as exact ISO so any phone time zone lines up), `/calendar/brief`, and `/calendar/suggest`: `readRequest` parses the words without a model, `understand` asks the model (`askJSON`, prompt starts `INTENT.`) and merges, `pickTimes` picks up to three real slots. The model only reads the request; code picks every time.
  - `features/nudges.js`: `/nudges` (friends and whether they have a push subscription), `/nudges/:uid`, `/nudges` (all), `/nudges/seen`. A nudge is a `kind: 'nudge'` message plus `users.nudged_at/nudged_by`, which `/me` returns so `PushPrompt` asks at once.
  - `ai.js` `chat()` tries the main model, then `LLM_FALLBACK_*` if set. `httpError` never keeps an HTML page; `aiErrorText` turns failures into a plain reason (ngrok offline, wrong URL, bad key).
...
- `web/` is React, Vite and react-router, with plain CSS in `web/src/styles.css`. Tokens are at the top. Dark mode is `:root[data-theme='dark']` plus `@media (prefers-color-scheme: dark) { :root:not([data-theme='light']) }`: any new dark-only rule needs both. Accents are `:root[data-accent=…]`, text size `data-text`, wallpaper `data-wallpaper`.
  ...
  - Performance: drags (chat-row swipes, swipe to reply, banners) move elements directly in `requestAnimationFrame` and never re-render per frame. `MessageList` in `ChatRoom.jsx` is memoised on the messages, so typing doesn't redraw the chat. Theme changes go through `changeLook` (`lib/look.js`, a view transition with a circular reveal). Login and the invite sheet (QR) load lazily.
  - Calls: `Call.jsx` has a watchdog: not connected after 25s (8s once failed), lost for 15s, or offline for 8s ends the call with the reason and returns (`end(why, { fail: true })`).
