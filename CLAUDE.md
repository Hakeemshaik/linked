...
- `server/` is Node 22 and Express on Postgres, plus web-push (VAPID). The server is stateless: anything shared lives in the DB, so it runs as a Vercel function.
  ...
  - `api.js` has every REST route: auth, invite links, friends, availability, events, conversations and messages (reactions, replies, delete), typing, plan confirm, invites, call signalling (`/calls/:room/*`, WebRTC mesh, max 6), presence heartbeats, notifications, `/ai/*`, `/cron/reminders`, `/health`.
