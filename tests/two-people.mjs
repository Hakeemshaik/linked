...
await A.click('text=Invite friends'); await A.click('.invite-actions button:has-text("Share invite link")'); await A.waitForSelector('.toast:has-text("Invite link copied")');
...
// --- They chat ---
await Bp.fill('.composer textarea','heyy it worked 🙌 its Sipho'); await Bp.press('.composer textarea','Enter');
...
await A.fill('.composer textarea','yooo welcome! braai saturday?'); await A.press('.composer textarea','Enter');
await Bp.waitForSelector('text=yooo welcome'); log('Person 2 got the reply live');
await Bp.waitForTimeout(2100); await Bp.locator('.composer textarea').pressSequentially('say less, im in', {delay:60}); 
await A.waitForSelector('.chat-who small:has-text("typing")'); log('Person 1 sees "typing…"'); await shot(A,'08-a-typing');
await Bp.press('.composer textarea','Enter'); await A.waitForSelector("text=say less"); await A.waitForTimeout(800);
...
await A.goto(B+'/you'); await A.click('text=Sign out'); await A.click('.confirm button:has-text("Sign out")'); await A.waitForSelector('.welcome-actions'); 
